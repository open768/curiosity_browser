...
$.widget('ck.solcalendar', {
  ...
  _create: function () {
    ...
    if (sElementName !== 'DIV') { $.error('calendar view needs a DIV. this element is a: ' + sElementName) }
    oElement.uniqueId()
  ...
  prv__getData: function () {
    ...
    const sUrl = cBrowser.buildUrl(cLocations.rest + '/cal.php', { s: oOptions.sol, m: oOptions.mission.ID })
  ...
  prv__get_Headings: function (paDates) {
    ...
    for (sDateKey in paDates) { aHeadings.push(sDateKey) }
    return aHeadings
  },
  ...
  prv__get_Times: function (paDates) {
    ...
    for (sDate in paDates) {
      for (sTime in paDates[sDate]) {
        if (aTimes.indexOf(sTime) == -1) { aTimes.push(sTime) }
      }
    }
    aTimes.sort()
  ...
  prv__build_colour_part: function (paInstr) {
    ...
    for (i = 0; i < paInstr.length; i++) {
      ...
      const oInnerSpan = $('<span>').attr({ style: 'background-color:' + oInstr.colour })
      oInnerSpan.append('&nbsp;&nbsp;&nbsp;')
