...
$.widget('ck.instrumentimage', {
  ...
  _create: function () {
    ...
    if (sElementName !== 'DIV') { $.error('image view needs a DIV. this element is a: ' + sElementName) }
    ...
    const oWaitImg = $('<IMG>', { src: this.consts.WAIT_IMAGE, id: sWaitImgID })
  ...
  prv_loadDetails: function () {
    ...
    oElement.empty()
    oElement.addClass('ui-widget-content')
    let oDiv = $('<DIV>', { class: 'ui-widget-header' }).append('Loading details for: ' + oOptions.product)
    oElement.append(oDiv)
  ...
  onProductDetails: function (poHttp) {
    if (poHttp.response.d) {
      ...
      this.prv__render()
    } else { this.onProductError(poHttp) }
  },
