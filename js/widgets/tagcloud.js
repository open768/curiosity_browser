...
$.widget('ck.tagcloud', {
  ...
  process_response: function (poHttp) {
    ...
    iMax = 0
    for (sKey in oData) { iMax = Math.max(iMax, oData[sKey]) }
  ...
  onInView: function () {
    ...
    const oDiv = $('<div>', { class: 'ui-widget-header' }).append('Loading Tags...').append(oLoader)
    oElement.append(oDiv)

    const oHttp = new cHttp2()
    const sUrl = cBrowser.buildUrl(cLocations.rest + '/tag.php', { o: 'all', m: this.options.mission.ID })
