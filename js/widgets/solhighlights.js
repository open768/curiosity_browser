        ...
        var oHttp = new cHttp2();
        {
            if (cBrowser.data[cSpaceBrowser.MOSAIC_QUERYSTRING] != null) {
                ...
                bean.on(oHttp, 'result', (poHttp) =>
                    oThis.onMosaicResponse(poHttp),
                );
            } else {
                oParams[cSpaceBrowser.OUTPUT_QUERYSTRING] = 'soldata';
                bean.on(oHttp, 'result', (poHttp) =>
                    oThis.onSheetResponse(poHttp),
                );
            }
...
$.widget('ck.instrhighlight', {
    ...
    load_highlights: function (poSpan) {
        ...
        bean.on(oItem, 'result', (poHttp) =>
            oThis.onHighlightResponse(oItem, poHttp),
        );
        bean.on(oItem, 'error', (poHttp) =>
            oThis.onHighlightError(oItem, poHttp),
        );
