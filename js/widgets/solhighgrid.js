...
$.widget("ck.solhighgrid", {
   ...
   _create: function () {
      ...
      bean.on(oHttp, "result", (poHttp) => oThis.onHighlightsResponse(poHttp))
      const sUrl = cBrowser.buildUrl(cLocations.rest + "/img_highlight.php", {
         o: "topsolindex",
   ...
   onHighlightsResponse: function (poHttp) {
      ...
      if (oOptions.aSolWithHighs == null) {
         ...
         const sUrl = cBrowser.buildUrl(cLocations.rest + "/sols.php", {
            m: oOptions.mission.ID,
         })
         const oHttp = new cHttp2()
         bean.on(oHttp, "result", function (poHttp) {
            oThis.onSolsResponse(poHttp)
         })
         oHttp.fetch_json(sUrl)
      }
   },
   ...
   onButtonClick: function (poEvent) {
      ...
      const sUrl = cBrowser.buildUrl("solhigh.php", { s: sSol })
