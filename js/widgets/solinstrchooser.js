...
$.widget('ck.solinstrumentChooser', {
	...
	render_part_sols: function () {
		...
		var oWidgetHeader = $('<SPAN>')
		{
			...
			oWidgetHeader.append($('<SPAN>', { id: sOID }).append('choose a SOL...'))
		}
		...
		var oBody = oWidget.body
		{
			...
			oButtonDiv = $('<DIV>', { style: 'white-space:nowrap' })
			{
				...
				oButton = cAppRender.make_button(null, oIcon.prop('outerHTML') + ' <span>previous</span>', 'previous Sol [', false, () => oThis.onPreviousSolClick())
				oButtonDiv.append(oButton)

				sButID = sID + this.consts.LATEST_ID
				oButton = cAppRender.make_button(sButID, 'latest', 'latest sol', true, () => oThis.onLatestSolClick())
				oButtonDiv.append(oButton)
				//-----------------------------------------------------
				oIcon = cRenderGoogleFont.create_icon('arrow_forward_ios')
				oButton = cAppRender.make_button(null, '<span>next</span> ' + oIcon.prop('outerHTML'), 'next Sol ]', false, () => oThis.onNextSolClick())
				oButtonDiv.append(oButton)
	...
	get_sol_instruments: function (psSol) {
		...
		{
			bean.on(oHttp, 'result', poHttp => oThis.onLoadSolInstruments(poHttp))
	...
	prLoadLists: function () {
		...
		const oHttp = new cHttp2()
		{
			bean.on(oHttp, 'result', poHttp => oThis.onLoadInstruments(poHttp))
			oHttp.fetch_json(sUrl)
		}
	...
	onLoadInstruments: function (poHttp) {
		...
		if (cBrowser.data[cSpaceUrlParams.INSTRUMENT]) {
			const sInstr = cBrowser.data[cSpaceUrlParams.INSTRUMENT]
	...
	onLoadSols: function (poHttp) {
		...
		if (cBrowser.data[cSpaceUrlParams.INSTRUMENT]) {
			this.options.instrument = cBrowser.data[cSpaceUrlParams.INSTRUMENT]
		}
		if (cBrowser.data[cSpaceUrlParams.SOL]) {
			this.set_sol(cBrowser.data[cSpaceUrlParams.SOL])
		}
