...
$.widget('ck.imageview', {
  ...
  onFullImages: function (poHttp) {
    ...
    if (oJson.max == 0) { this.element.html('No data found') } else {
      // nothing in this div
      this.element.empty()
      ...
      let oButton = $('<button>', { class: 'leftarrow imagenav', title: 'Previous Page', id: sID + this.consts.LeftID })
      if (oOptions.start_image < this.consts.IMAGES_TO_SHOW) { oButton.attr('disabled', 'disabled') } else { oButton.html('Previous Page').click(function () {		oThis.onClickPreviousPage()	}) }
      ...
      if (oOptions.start_image >= oJson.max - this.consts.IMAGES_TO_SHOW) { oButton.attr('disabled', 'disabled') } else { oButton.html('Next Page').click(function () {		oThis.onClickNextPage()	}) }
      oCell.append(oButton)
      oRow.append(oCell)
