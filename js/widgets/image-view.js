...
class cImageList {
	...
	load_images(piStartImage) {
		...
		var sCaption = 'Loading Images for sol:' + oOptions.sol + ' instr:' + oOptions.instrument
		const oDiv = cAppRender.make_spinner(sCaption)
		oElement.append(oDiv)

		// --------------update the Browser url
		var oParams = {}
		oParams[cAppUrlParams.BEGIN_PARAM] = piStartImage
		oParams[cSpaceUrlParams.SOL] = oOptions.sol
		oParams[cSpaceUrlParams.INSTRUMENT] = oOptions.instrument
	...
	pr_render_navigation(piMax) {
		...
		})
		{
			...
			var oPrevPageButton = cAppRender.make_button(null, '<< Previous', 'previous Page', bDisabled, () => oThis.onClickPreviousPage())
			{
			...
			var oNextPageButton = cAppRender.make_button(null, 'Next >>', 'next Page', bDisabled, () => oThis.onClickNextPage())
			{
	...
	onClickNextPage() {
		const oOptions = this.options
		if (oOptions.start_image >= oOptions.max_images - this.IMAGES_TO_SHOW) return
