...
function onClickNextTime () {
  const sUrl = cBrowser.buildUrl(cLocations.rest + '/nexttime.php', { d: 'n', s: goItem.s, p: goItem.p, m: cMission.ID })
...
function onClickPreviousTime () {
  const sUrl = cBrowser.buildUrl(cLocations.rest + '/nexttime.php', { d: 'p', s: goItem.s, p: goItem.p, m: cMission.ID })
...
function onClickPrevious () {
  cCommonStatus.set_status('fetching previous image details...')
  const sUrl = cBrowser.buildUrl(cLocations.rest + '/next.php', { d: 'p', s: goItem.s, i: goItem.i, p: goItem.p, m: cMission.ID })
  const oHttp = new cHttp2()
  bean.on(oHttp, 'result', next_callback)
  oHttp.fetch_json(sUrl)
}
...
//* **************************************************************
function onClickMap () {
  const sUrl = 'http://curiosityrover.com/imgpoint.php?name=' + goItem.p
  window.open(sUrl, 'map')
}

//* **************************************************************
function onClickSol () {
  const sUrl = 'index.php?s=' + goItem.s + '&i=' + goItem.i + '&b=' + giNum
  cBrowser.openWindow(sUrl, 'index')
}

//* **************************************************************
function onClickThumbnails () {
  const sUrl = 'index.php?s=' + goItem.s + '&i=' + goItem.i + '&t=1'
  cBrowser.openWindow(sUrl, 'solthumb')
}

//* **************************************************************
function onClickHighlights () {
  const sUrl = 'solhigh.php?sheet&s=' + goItem.s
  cBrowser.openWindow(sUrl, 'solthumb')
}
...
function onClickMSLRaw () {
  const sUrl = 'http://mars.nasa.gov/msl/multimedia/raw/?rawid=' + goItem.p + '&s=' + goItem.s
  window.open(sUrl, 'mslraw')
}

//* **************************************************************
function onClickPDS () {
  const sUrl = 'pds.php?s=' + goItem.s + '&i=' + goItem.i + '&p=' + goItem.p + '&t=' + escape(goItem.d.du)
...
function onClickGoogle () {
  const sUrl = 'https://www.google.com/#q=%22' + goItem.p + '%22'
  window.open(sUrl, 'map')
}
...
function alltagnames_callback (poJs) {
  ...
  gaTags = new Array()
  for (sKey in poJs) { gaTags.push(sKey) }
...
function tag_callback (paJS) {
  ...
  if (paJS.d.length == 0) {
    ...
    for (i = 0; i < paJS.d.length; i++) {
      ...
      const sTarget = (SINGLE_WINDOW ? '' : "target='tags'")
...
function load_detail_callback (poHttp) {
  ...
  if (oData === null) {
    ...
    } else { cBrowser.openWindow('error.php?m=product ' + goItem.p + ' was not found', 'error') }
    return
  }
  ...
  // add the image
  $('#image').empty()
  const oImg = $('<img/>').attr({ src: oData.i, id: 'baseimg', onload: 'OnImageLoaded()' })
  $('#image').append(oImg)
...
function nexttime_callback (poHttp) {
  ...
  if (!oData) { cCommonStatus.set_error_status('unable to find') } else { get_product_data(oData.s, oData.d.instrument, oData.d.itemName) }
}
...
function onClickBoxAccept () {
  const oBox = cImgHilite.getBoxFromButton(event.currentTarget)
