...
class cDetail {
  ...
  static onClickComment() {
    ...
    cComments.set(this.oItem.s, this.oItem.i, this.oItem.p, sText, () => this.onGotComments())
  }

  //* **************************************************************
  static onClickNextProduct() {
    const sUrl = cBrowser.buildUrl(cLocations.rest + '/nexttime.php', { d: 'n', s: this.oItem.s, p: this.oItem.p, m: cMission.ID })
  ...
  static onClickPreviousProduct() {
    const sUrl = cBrowser.buildUrl(cLocations.rest + '/nexttime.php', { d: 'p', s: this.oItem.s, p: this.oItem.p, m: cMission.ID })
  ...
  static onClickNext() {
    ...
    cCommonStatus.set_status('fetching next image details...')
    var sUrl = cBrowser.buildUrl(cLocations.rest + '/next.php', { d: 'n', s: this.oItem.s, i: this.oItem.i, p: this.oItem.p, m: cMission.ID })
    const oHttp = new cHttp2()
  ...
  static onClickPrevious() {
    cCommonStatus.set_status('fetching previous image details...')
    const sUrl = cBrowser.buildUrl(cLocations.rest + '/next.php', { d: 'p', s: this.oItem.s, i: this.oItem.i, p: this.oItem.p, m: cMission.ID })
    const oHttp = new cHttp2()
    bean.on(oHttp, 'result', (oData) => this.next_callback(oData))
    oHttp.fetch_json(sUrl)
  }
  ...
  //* **************************************************************
  static onClickSol() {
    const sUrl = 'index.php?s=' + this.oItem.s + '&i=' + this.oItem.i + '&b=' + this.iNum
  ...
  static onClickMSLRaw() {
    const sUrl = 'http://mars.nasa.gov/msl/multimedia/raw/?rawid=' + this.oItem.p + '&s=' + this.oItem.s
    window.open(sUrl, 'mslraw')
  }

  //* **************************************************************
  static onClickPDS() {
    const sUrl = 'pds.php?s=' + this.oItem.s + '&i=' + this.oItem.i + '&p=' + this.oItem.p + '&t=' + escape(this.oItem.d.du)
  ...
  static onClickAddTag() {
    ...
    cTagging.setTag(this.oItem.s, this.oItem.i, this.oItem.p, sTag, () => this.onSetTag())
  }

  //* **************************************************************
  static onClickPixlr() {
    /* global pixlr */
    pixlr.edit({ image: this.oItem.d.i, service: 'editor', exit: document.location, referer: 'mars browser', redirect: false })
  }
  ...
  static onKeyPress(poEvent) {
    ...
    switch (sChar) {
      case 'n': this.onClickNext(); break
      case 'N': this.onClickNextProduct(); break
      case 'p': this.onClickPrevious(); break
      case 'P': this.onClickPreviousProduct(); break
    }
  }
  ...
  static OnImageClick(poEvent) {
    if (cAuth.user) { cImgHilite.makeBox(poEvent.pageX, poEvent.pageY, true) } else { alert('log in to highlight') }
  }

  //* *************************************************
  static onClickBoxAccept(poEvent) {
    const oBox = cImgHilite.getBoxFromButton(poEvent.currentTarget)
    cImgHilite.save_highlight(this.oItem.s, this.oItem.i, this.oItem.p, oBox, () => this.onSaveHighlight())
  }
  ...
  static onFacebookUser() {
    ...
    $('#Commentsbox').sceditor('instance').blur(() => this.onInputDefocus())
    $('#Commentsbox').sceditor('instance').focus(() => this.onInputFocus())
  }
  ...
  static onGotAllTagNames(poJs) {
    ...
    for (var sKey in poJs) { this.aTags.push(sKey) }
  ...
  static load_detail_callback(poHttp) {
    ...
    if (oData === null) {
      if (this.oItem.migrate !== null) {
        sUrl = 'migrate.php?s=' + this.oItem.s + '&i=' + this.oItem.i + '&pfrom=' + this.oItem.p + '&pto=' + this.oItem.migrate
        cBrowser.openWindow(sUrl, 'migrate')
      } else { cBrowser.openWindow('error.php?m=product ' + this.oItem.p + ' was not found', 'error') }
      return
    }

    // update the title
    document.title = 'detail: s:' + this.oItem.s + ' i:' + this.oItem.i + ' p:' + this.oItem.p + ' (Curiosity Browser)'
    $('#toptitle').html(this.oItem.p)

    // update the address bar
    sUrl = cBrowser.pageUrl() + '?s=' + this.oItem.s + '&i=' + this.oItem.i + '&p=' + this.oItem.p
    cBrowser.pushState('Detail', sUrl)

    // tags
    if (!oData.tags) { $('#tags').html('no Tags - be the first to add one') } else {
  ...
  static OnImageLoaded(poEvent) {
    ...
    cImgHilite.getHighlights(this.oItem.s, this.oItem.i, this.oItem.p, (oData) => this.onGotHighlights(oData))
