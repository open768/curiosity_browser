...
class cBackgroundImage {
	...
	static OnImageResponse(poHttp) {
		...
		var oBut = cAppRender.make_button(null, 'refresh', 'refresh random background image', false, () => oThis.fetch_image())
		...
		})
		{
			...
			})
			{
				...
				oSolChooser.solinstrumentChooser({
					onStatus: (poEvent, paHash) => cIndexPage.onStatusEvent(poEvent, paHash),
					onSelect: (poEvent, poData) => cIndexPage.onSelectSolInstrEvent(poEvent, poData),
					mission: cMission
				})
			}
			//----------SOL buttons
...
class cAdminBox {
	...
	static async render(poParent) {
		...
		var oBody = oWidget.body
		{
			if (cIndexPageConsts.IS_ADMIN === 'yes') {
				var oButton = cAppRender.make_button(null, 'Admin', 'Admin functions', false, () => oThis.onAdminClick())
	...
	//*************************************************************
	static onAdminClick() {
		cBrowser.openWindow(cAppLocations.home + '/php/pages/admin/admin.php', 'admin')
	}
}
...
class cSearchBox {
	...
	static async render() {
		...
		//--------- raw images
		oElement.append(cBrowser.whitespace(50))
		oButton = cAppRender.make_button(cIndexPage.RAW_BUTTON_ID, 'Raw Images', 'See Raw images for this sol', true, () => cIndexPage.onClickRawImage())
...
class cIndexPage {
	...
	static onLoadJQuery() {
		...
		cJquery.element(cIndexPageConsts.ID_CHKTHUMBS).attr('disabled', 'disabled')
	}
	...
	static onClickAllSolThumbs() {
		...
		cJquery.element(cIndexPageConsts.ID_CHKTHUMBS).prop('checked', true).attr('disabled', 'disabled')
		cJquery.element(cLeftColumn.ID_WIDGET_SOLCHOOSER).solinstrumentChooser('deselectInstrument')
		this.load_data()
	}
	...
	static load_data() {
		...
		cDebug.write('loading data: ' + cIndexPageOptions.sol + ':' + cIndexPageOptions.instrument)
		...
		if (cIndexPageOptions.instrument) {
			...
			if (cBrowser.data[cSpaceBrowser.THUMB_QUERYSTRING]) {
				...
				this.show_images(cIndexPageOptions.sol, cIndexPageOptions.instrument, cIndexPageOptions.start_image)
			}
	...
	static show_thumbs(psSol, psInstrument) {
		var oDiv
		cAppRender.update_title('showing  thumbs for sol:' + psSol + ' : ' + psInstrument)
		const oThis = this
	...
	static show_images(piSol, psInstr, piStartImage) {
		var oWidget, oDiv
		const oThis = this
		cAppRender.update_title('showing  images for sol:' + piSol + ' : ' + psInstr)
