...
function all_sites_callback(poHttp) {
  ...
  $('#sites').empty()
  if (oData.d == null) { cCommonStatus.set_error_status('No sites found') } else {
    aSites = oData.d
...
function render_sites() {
  ...
  const sUrl = cBrowser.buildUrl('../hirise', { o: 'intersect', la1: oBounds.lat1, lo1: oBounds.long1, la2: oBounds.lat2, lo2: oBounds.long2 })
  const oHttp = new cHttp2()
...
function lookat_callback() {
  ...
  for (var i = 0; i < aSites.length; i++) {
    const sUrl = cBrowser.buildUrl(cLocations.rest + '/sites.php', { o: 'site', site: i, m: cMission.ID })
    const oHttp = new cHttp2()
...
function onClickSite() {
  ...
  sVal = $(this).val()
  if (sVal === 'reset') { cGoogleEarth.flyTo(oCentre.lat, oCentre.lon, 9000.0) } else {
    iSite = parseInt(sVal)
...
function onClickHirise() {
  ...
  if (oSelectedHiRise != null) { cGoogleEarth.setLineColour(oSelectedHiRise, sHiriseColor) }
